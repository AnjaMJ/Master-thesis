...
option_list = list(
              ...
              make_option(c("-m", "--MAF"), type="character", default=NULL, help="Minor allele freq"),
              make_option(c("-p", "--pop"), type="character", default=NULL, help="Population to select: 1 or 2")
    );
...
ChiSquared <- function(leaves_freqs,effects,F_mat,randomize=FALSE){
    ...
    pop_select <- as.numeric(pop_select)
    # P2 between chosen MAF and 0.95
    checkseg <- which( leaves_freqs[,pop_select] < 0.95  & leaves_freqs[,pop_select] > as.numeric(maf) )
